...
use super::{UpdateCheck, parse_release};
...
fn a_newer_release_with_both_assets_is_offered() {
    let body = release_json("v0.2.0", &[UPDATE_ASSET_NAME, CHECKSUM_ASSET_NAME]);

    let UpdateCheck::Available(update) = parse_release(&body, &v("0.1.0")).unwrap() else {
        panic!("새 버전을 제공해야 한다");
    };
...
fn the_same_or_older_release_is_up_to_date() {
    let body = release_json("v0.1.0", &[UPDATE_ASSET_NAME, CHECKSUM_ASSET_NAME]);
    assert_eq!(
        parse_release(&body, &v("0.1.0")).unwrap(),
        UpdateCheck::UpToDate
    );
    assert_eq!(
        parse_release(&body, &v("0.9.0")).unwrap(),
        UpdateCheck::UpToDate
    );
}
...
fn a_tag_without_the_v_prefix_is_handled() {
    let body = release_json("0.2.0", &[UPDATE_ASSET_NAME, CHECKSUM_ASSET_NAME]);
...
fn a_release_with_only_the_full_zip_is_unsupported_not_up_to_date() {
    let body = release_json("v0.2.0", &["anemone_rs-v0.2.0-full.zip"]);

    let result = parse_release(&body, &v("0.1.0")).unwrap();
...
fn a_release_without_the_checksum_is_unsupported() {
    let body = release_json("v0.2.0", &[UPDATE_ASSET_NAME]);
    assert!(matches!(
        parse_release(&body, &v("0.1.0")).unwrap(),
...
fn partial_name_matches_are_not_accepted() {
    ...
    let body = release_json("v0.2.0", &["anemone_rs-i686-pc-windows-msvc.exe.bak"]);
    assert!(matches!(
        parse_release(&body, &v("0.1.0")).unwrap(),
        UpdateCheck::Unsupported { .. }
    ));
...
fn asset_urls_outside_the_allowed_hosts_are_rejected() {
    let body = release_json("v0.2.0", &[UPDATE_ASSET_NAME, CHECKSUM_ASSET_NAME]).replace(
...
fn malformed_payloads_are_reported_as_parse_errors() {
    for body in ["", "{", "{}", r#"{"tag_name": "not-a-version"}"#] {
        assert!(
            matches!(parse_release(body, &v("0.1.0")), Err(UpdateError::Parse(_))),
            "{body:?}는 파싱 오류여야 한다"
        );
    }
}
